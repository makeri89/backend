...
def clean_ancient_data(wrecks_ancient):
    ...
    # change coordinate system to wgs84 which is usually the deafult in web browsers
...
def clean_wrecks_all_data(wrecks_all_collection):
    print(len(wrecks_all_collection), 'items in collection')
    print(wrecks_all_collection.columns)

    # select only underwater wrecks (marked as 'k' or 'K' in column Vedenalain)
    wrecks_all = wrecks_all_collection.loc[((wrecks_all_collection['Vedenalain'] == 'k') | (wrecks_all_collection['Vedenalain'] == 'K'))]
    print(len(wrecks_all), 'items in underwater collection')

    # change object Z Point to Point
    wrecks_all['geometry'] = wrecks_all['geometry'].apply(z_to_point)

    wrecks_all = wrecks_all.rename(columns={'Mjtunnus':'id', 'Kunta':'town', 'Tyyppi':'type', 'Kohdenimi':'name', 'Luontipvm':'created_at','Paikannust':'location_accuracy'})
    wrecks_all_cut = wrecks_all.loc[:, ['id', 'town', 'name', 'created_at', 'type', 'location_accuracy', 'geometry']]
...
def clean_union_data(wrecks_union, crs):
    # combine geometries, primary geo from wrecks_all, if missing, taking it from wrecks_ancient
    wrecks_union['geometry'] = wrecks_union.apply(lambda row: row['geometry_y'] if (row['geometry_x'] == None) else row['geometry_x'], axis=1)

    # fill in url to kyppi info page by wreck_ancient or create url by id
    wrecks_union['url'] = wrecks_union['url'].fillna('None')
    wrecks_union['url'] = wrecks_union.apply(lambda row: f"www.kyppi.fi/to.aspx?id=112.{row['id']}" if (row['url'] == 'None') else row['url'], axis=1)
    ...
    wrecks_union['name'] = wrecks_union.apply(lambda row: f"{row['Kohdenimi'].strip()}" if (row['name'] == 'None') else row['name'], axis=1)
    wrecks_union['town'] = wrecks_union['town'].fillna('None')
    wrecks_union['town'] = wrecks_union.apply(lambda row: f"{row['Kunta'].strip()}" if (row['town'] == 'None') else row['town'], axis=1)
    wrecks_union['town'] = wrecks_union.apply(lambda row: np.nan if (row['town'] == 'ei kuntatietoa') else row['town'], axis=1)
    ...
    wrecks_union['location_accuracy'] = wrecks_union.apply(lambda row: np.nan if ((row['location_accuracy'] == 'null') | (row['location_accuracy'] == 'Ei tiedossa')) else row['location_accuracy'], axis=1)
    ...
    wrecks_union = wrecks_union.drop(['geometry_x', 'geometry_y', 'OBJECTID', 'inspireID', 'Kohdenimi',
       'Kunta', 'Laji', 'tyyppi', 'alatyyppi', 'ajoitus', 'vedenalainen',
       'luontipvm', 'muutospvm', 'paikannustapa', 'paikannustarkkuus',
       'selite', 'x', 'y'], axis=1)
...
def load_and_clean_data(path):
    ...
    url_ancient = 'https://kartta.nba.fi/arcgis/services/WFS/MV_HylytMeritietoportaali/MapServer/WFSServer?service=WFS&request=GetFeature&typeName=WFS_MV_HylytMeritietoportaali:Alusten_hylyt&outputFormat=GEOJSON'

    # fetch data from WFS-server
    try:
        r = requests.get(url_ancient)
    except requests.exceptions.RequestException:
        print('connection error when fetching from kartta.nba.fi')

    # create GeoDataFrame from fetched geojson-data
    wrecks_ancient = gpd.GeoDataFrame.from_features(geojson.loads(r.content))
   
    # fetch data of all wrecks and ruins as ShapeFile collection to data/wrecks_all folder
    load_and_unpack_zip_file('https://paikkatieto.nba.fi/aineistot/tutkija.zip', path)

    # read in data of all remains (underwater and not underwater)
    wrecks_all_collection = gpd.read_file(os.path.join(path, 'wrecks_all/Muinaisjaannospisteet_t_point.shp'))
...
def delete_temp_folder(path):
    try:
        shutil.rmtree('data/wrecks_all')
        os.remove('data/tutkija.zip')
    except OSError as e:
        print(f'Error when removing temporary files: {e.filename} - {e.strerror}.')
