...
def clean_union_data(wrecks_union, crs):
    ...
    wrecks_union['geometry'] = wrecks_union.apply(lambda row: row['geometry_y'] \
    ...
    wrecks_union['name'] = wrecks_union.apply(lambda row: f"{row['Kohdenimi'].strip()}" \
                                                if (row['name'] == 'None') \
    ...
    wrecks_union['location_accuracy'] = wrecks_union.apply(lambda row: np.nan \
                                                if ((row['location_accuracy'] == 'null') | \
                                                    (row['location_accuracy'] == 'Ei tiedossa')) \
                                                else row['location_accuracy'], axis=1)
...
def delete_temp_folder(path):
    try:
        ...
        print(f'Error when removing temporary files: {exp.filename} - {exp.strerror}.')
